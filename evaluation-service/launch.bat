...
if "%*" == "" (
	...
	for %%s in ("-model" "-kjar" "evaluation-service") do (
...
:startapp
	if not x%mavenInput:docker=%==x%mavenInput% (
		...
		call docker run -d -p 8090:8090 --name evaluation-service apps/evaluation-service:1.0-SNAPSHOT
	) else if not x%mavenInput:openshift=%==x%mavenInput% (
		echo Launching the application on OpenShift...
		call oc new-app evaluation-service:1.0-SNAPSHOT
		call oc expose svc/evaluation-service
	) else (
