...
if [ "$MVN_ARG_LINE" != "" ] ; then
    ...
    "$mvnBin" -v
    echo
    projects=( "*-model" "*-kjar" "evaluation-service")
...
if [[ "$@" =~ "docker" ]]; then
    ...
    docker run -d -p 8090:8090 --name evaluation-service apps/evaluation-service:1.0-SNAPSHOT
elif [[ "$@" =~ "openshift" ]]; then
    echo "Launching the application on OpenShift..."
    
    oc new-app evaluation-service:1.0-SNAPSHOT
    oc expose svc/evaluation-service
else

	echo "Launching the application locally..."
	pattern="evaluation-service"
	files=( $pattern )
	cd ${files[0]}
